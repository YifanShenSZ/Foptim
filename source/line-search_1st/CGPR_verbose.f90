...
subroutine CGPR_verbose(f, f_fd, x, dim, &
...
real*8::precision_square, min_StepLength_square, a, fnew, fdoldfdold, fdnewfdnew, phidold, phidnew
...
do iIteration = 1, max_iteration
    ...
    if (dot_product(p, p) * a * a < min_StepLength_square) then
        write(*,*)"Polak-Ribiere+ conjugate gradient warning: step length has converged, but gradient norm has not met accuracy goal"
        write(*,*)"Euclidean norm of gradient =", dSqrt(fdnewfdnew)
        return
    end if
    !determine new direction
    p = -fdnew + dot_product(fdnew, fdnew - fdold) / fdoldfdold * p
...
if (iIteration > max_iteration) then
    write(*,*)"Failed Polak-Ribiere+ conjugate gradient: max iteration exceeded!"
    write(*,*)"Final ||gradient|| = ", norm2(fdnew)
end if

end subroutine CGPR_verbose